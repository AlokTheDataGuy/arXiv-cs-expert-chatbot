...
export const ChatProvider = ({ children }) => {
  ...
  const sendMessage = async (content) => {
    ...
    try {
      ...
      const response = await apiClient.sendMessage(content, 'default-user');

      // Create assistant message from API response
      const assistantMessage = {
        id: response.id || (Date.now() + 1).toString(),
        role: 'assistant',
        content: response.response || 'No response from the server.',
        timestamp: response.timestamp || new Date().toISOString(),
        papers: response.relevant_papers || []
      };
      ...
      if (process.env.NODE_ENV === 'development') {
        errorContent = generateMockResponse(content);
